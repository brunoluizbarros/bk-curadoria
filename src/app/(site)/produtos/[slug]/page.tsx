import { getProductBySlug, getActiveProducts } from "@/server/queries/products";
...
export default async function ProdutoPage({ params }: Props) {
  ...
  const firstImageUrl = product.images[0]?.url ?? null;
  ...
  const breadcrumbs = [
    ...
    { name: "Loja", url: `${BASE}/categorias` },
    { name: product.name, url: `${BASE}/produtos/${slug}` },
  ];
  ...
  return (
    <>
      ...
      <div className="max-w-[480px] md:max-w-2xl lg:max-w-4xl mx-auto px-0 md:px-6 py-6 md:py-10">
        ...
        <div className="flex justify-between items-center px-5 md:px-0 mb-4">
          <Breadcrumbs items={[{ label: "Início", href: "/" }, { label: "Loja", href: "/categorias" }, { label: product.name }]} />
          <BackOrHome />
        </div>
